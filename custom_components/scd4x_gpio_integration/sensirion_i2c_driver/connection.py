...
class I2cConnection(object):
    ...
    def is_multi_channel(self):
        ...
        if self._transceiver.API_VERSION == 1:
            ...
            raise Exception("The I2C transceiver API version {} is not "
                            ...
                            "sensirion-i2c-driver package.".format(self._transceiver.API_VERSION))

    async def async_execute(self, slave_address, command, wait_post_process=True):
        """
        ...
        """
        response = await self._async_transceive(
        ...
        if wait_post_process and command.post_processing_time > 0.0:
            ...
            await asyncio.sleep(command.post_processing_time)
        return self._interpret_response(command, response)

    async def _async_transceive(self, slave_address, tx_data, rx_length, read_delay,
                                timeout):
        """
        API version independent wrapper around the transceiver.
        """
        api_methods_dict = {
            1: self._async_transceive_v1,
        }
        if self._transceiver.API_VERSION in api_methods_dict:
            ...
            result = await api_methods_dict[self._transceiver.API_VERSION](
    ...
    async def _async_transceive_v1(self, slave_address, tx_data, rx_length, read_delay,
                                   timeout):
        """
        Helper function to transceive a command with a API V1 transceiver.
        """
        result = await self._transceiver.async_transceive(
            slave_address=slave_address,
            tx_data=tx_data,
            rx_length=rx_length,
